create table customer(customerId varchar(100) primary key,name varchar(40),address varchar(50),locality varchar(20),landmark varchar(20),city varchar(15),state varchar(20),accountType varchar(10) check(accountType in('simple','golden')),emailId varchar(70),contno varchar(15),password varchar(150),accountDate date);
...
create table homeDelivery(homeDeliveryId int primary key,homeDeliveryAddress varchar(100),orderId int references order1(orderId)on delete cascade on update cascade,deliveryboyid int references deliveryboy(deliveryboyid)on delete cascade on update cascade,unique(homeDeliveryId,orderId,deliveryboyid),unique(orderId));
